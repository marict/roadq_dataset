...
from osm_utils import GeoPoint, Node, Edge, StreetSegment, get_segment_key
...
def prepare_seattle_street():
    df = pd.read_csv(SEATTLE_STREET_CSV_ORIGINAL, usecols=['XSTRLO', 'XSTRHI', 'ONSTREET', 'PVMTCONDINDX1', 'PVMTCONDINDX2', 'UNITDESC'])
...
@click.option('--file', help='File to be used. If not provided, the last generated will be used.')
...
def read_segments_from_seattle_data():
    ...
    for _, entry in df.iterrows():
        ...
        segment = StreetSegment(entry['PCI1'], street_name, [entry['ROAD1'], entry['ROAD2']])
