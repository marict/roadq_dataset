...
def parse_args():
    parser = argparse.ArgumentParser(description='Parse validation and predictions CSV files')
    parser.add_argument('--validation-csv', default = "validation/dummy_validation_data.csv", type=str, help='Path to the validation CSV file')
    parser.add_argument('--latitude-resolution', default=0.005, type=float, help='Latitude resolution')
    parser.add_argument('--longitude-resolution', default=0.005, type=float, help='Longitude resolution')
    return parser.parse_args()

def resample_data(df: pd.DataFrame, latitude_resolution: float, longitude_resolution: float) -> pd.DataFrame:
    """Resample the data to the nearest latitude and longitude resolution."""
    df['QUANTIZED_LAT'] = (np.floor(df['LATITUDE'] / latitude_resolution) * latitude_resolution)
    df['QUANTIZED_LON'] = (np.floor(df['LONGITUDE'] / longitude_resolution) * longitude_resolution)
    print(f"Resampled data from {len(df)} to {len(df.groupby(['QUANTIZED_LAT', 'QUANTIZED_LON']))} rows")
...
def get_predictions_(validation_csv: pathlib.Path) -> pd.DataFrame:
    ...
    predictions = []
    for timestamp, lat, lon, pci in tqdm(val_df[['TIMESTAMP', 'LATITUDE', 'LONGITUDE', 'PCI']].values, total=len(val_df), unit='row'):
        ...
        predictions.append({'TIMESTAMP': timestamp, 'LATITUDE': lat, 'LONGITUDE': lon, 'PCI': pci, 'PCI_pred': pci_pred})
